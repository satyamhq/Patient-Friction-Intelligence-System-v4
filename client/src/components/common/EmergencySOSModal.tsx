...
export const EmergencySOSModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    ...
    >
      <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-rose-500/60 w-full max-w-lg p-5 sm:p-7 shadow-2xl space-y-4 relative my-auto max-h-[92vh] overflow-y-auto shrink-0">
        ...
        {dispatchData ? (
          <div className="space-y-4 animate-scale-up">
            <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-300 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-emerald-200">
                <div>
                  ...
                  <strong className="text-emerald-700 text-base font-extrabold flex items-center gap-1">
                    <Clock className="w-4 h-4" /> ~{dispatchData.etaMinutes} Minutes
                  </strong>
                </div>
                <div>
                  <span className="text-slate-500 block">Direct Emergency Line</span>
                  <strong className="text-slate-900 text-base font-extrabold flex items-center gap-1">
            ...
            <button
              onClick={onClose}
              className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs shadow cursor-pointer transition-all"
            >
          ...
          <div className="space-y-4 text-xs">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-semibold"
              >
            ...
            <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 space-y-1 text-slate-700">
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full py-3 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 font-semibold text-xs transition-colors cursor-pointer text-center"
            >
              Cancel & Close Window
            </button>
          </div>
